...
const generateApiKeys = asyncHandler(async (req, res) => {
    ...
    const organization = await Organization.findById(req.params.id);
    // We need the full user object to access the matchPassword method
    ...
    // --- KEY GENERATION ---
    const publicKey = `Ap_live_${crypto.randomBytes(16).toString('hex')}`;
    ...
    res.status(200).json({
        ...
        publicKey,
        secretKey // Return the unhashed secret key ONCE
    });
});
